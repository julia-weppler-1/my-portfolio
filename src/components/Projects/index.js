...
const projects = [
    ...
    },
    {
        ...
        image: [{url: '/Scoring Assistant.png', description: ''}],
    ...
    },
    {
        ...
        image: [{url: '/data dash filters.png', description: 'First, you can see users have the option to add data views by source (survey questions versus characteristics), and modify the data included by year. They can also choose if the data is represented in percentages or counts, and specify AND versus OR based filtering when filtering by responses (Any versus All).'}, 
            {url: '/data dash dot plot.png', description: 'Integrated in the dashboard is one correlogram-heatmap hybrid graph, where users can choose which x and y fields they\'d like to display. The component will then decipher if the relationship between these variables is significant by performing a chi-squared test.'},
            {url: '/data dash bars unfiltered.png', description: 'Underneath is the graphs that the user selected, with a distinction of the source (survey questions versus characteristics). None of the graphs have been filtered yet. The user can also toggle between 3 graph types for each visual.'},
            {url: '/data dash customize view.png', description: ''}, 
            {url: '/data dash 1 filter.png', description: 'Here is the same dashboard when a user clicks a bar to filter across all data with that response:'}, 
            {url: 'data dash multiple filters, tooltip.png', description: 'And again filtering using AND with another response. You can also see that each element imn a graph has a tooltip for more precise information:'}],
    ...
    },
    {
        ...
        image: [{url: '/EagleApps Login.png', description: 'First, users will be prompted to login using Google SSO with their Boston College Credentials.'},
                {url: '/EagleApps Year Select.png', description: 'Next, if they are a new user, they will be prompted to fill out the following information. They will not be able to continue without finishing the required fields.'},
                {url: '/EagleApps Progress.png', description: 'If they are a returning user, they will instead be brought to the landing page. If they already have courses in their plan, they will see their credit progress.'},
                ...
                {url: '/EagleApps Course Search.png', description: 'Next, if the user navigates to "Search Courses", they will be able to find courses by department. In future work on this progress, it would be helpful to implement a fully-functional free-text search, though that was out of the scope of this project.'},
                {url: '/EagleApps Add Course.png', description: 'Students can add a desired course to thier plan by specifying what year and semester they\'d like to include it in.'},
                ...
                {url: '/EagleApps Dept View.png', description: 'If a user is an administartor and would like a department-view, they will be able to see how many students have a class in their plan by semester and year, so that the University can estimate how many seats they will need for that course.'},
                ],
